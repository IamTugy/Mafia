...
export const useServerStore = create<ServerStore>((set, get) => ({
  ...
  initializeGame: () => {
    ...
    const roles: Role[] = ['don', 'mafia', 'mafia', 'sheriff', 'civilian', 'civilian', 'civilian', 'civilian', 'civilian', 'civilian'];
    const shuffledRoles = [...roles].sort(() => Math.random() - 0.5);
    
    set(state => ({
      clients: state.clients.map((client) => {
        if (client.playerData.status === 'inGame') {
          const gameIndex = inGameClients.findIndex(c => c.playerData.id === client.playerData.id);
          return {
            ...client,
            playerData: {
              ...client.playerData,
